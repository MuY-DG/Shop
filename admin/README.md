...
在「配置管理 → 微信平台配置 → 小程序展示设置」修改，初始名称为「蜀香序」。
名称独立保存，无需重新填写或验证 AppSecret；沿用微信平台配置的读写权限。
接口为 `GET/PUT /admin/display-config`，保存携带版本号以防止覆盖其他管理员的修改。
客户端刷新配置后用于页面与分享；后端只在首次准备新支付时保存名称快照，
已有支付、重试和历史记录保留原描述。此设置不会修改微信平台的正式名称。

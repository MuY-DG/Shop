...
后台「配置管理 → 微信平台配置 → 小程序展示设置」维护统一展示名称，初始值为「蜀香序」。
小程序通过免登录的 `GET /app/display-config` 获取名称；启动或重新进入前台时刷新，
相关页面出现时也会刷新（同一分钟内合并），名称按当前环境存入本地缓存。
无缓存或接口暂不可用时，使用 `app-config.ts` 中的 `appName` 兜底，页面加载不等待配置。
页面、分享标题和无障碍描述统一读取 `services/display-config.ts`，不要直接使用默认名称。
微信原生隐私指引仍优先显示微信返回的名称；已发布法律文档正文不随展示名称自动改写。

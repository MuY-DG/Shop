...
function readSearchHistory(): string[] {
  try {
    return normalizeProductSearchHistory(wx.getStorageSync(PRODUCT_SEARCH_HISTORY_KEY));
  } catch {
    return [];
  }
}
...
Page({
  ...
  onLoad(options: SearchPageOptions) {
    this.setData({
      keyword: normalizeProductRouteKeyword(options.keyword),
  ...
  onClearHistory() {
    wx.showModal({
      ...
      success: (result) => {
        ...
        try {
          wx.removeStorageSync(PRODUCT_SEARCH_HISTORY_KEY);
        } catch {
  ...
  submitSearch(value: unknown) {
    ...
    const history = addProductSearchHistory(readSearchHistory(), keyword);
    try {
      wx.setStorageSync(PRODUCT_SEARCH_HISTORY_KEY, history);
    } catch {
      // Search should remain available even when local storage is full.
    }
    this.setData({ keyword, history });
    wx.redirectTo({
      url: `/pages/product/list/list?keyword=${encodeURIComponent(keyword)}`
    });
  }
});

...
async function performRenewal(preferRefresh: boolean): Promise<SessionState> {
  ...
  if (preferRefresh && state.refreshToken) {
    try {
...
export async function ensureSession(): Promise<SessionState> {
  ...
  return renewSession(Boolean(current.refreshToken));
}

export function silentLogin(): Promise<SessionState> {
  restoreSession();

...
test("开发版保留现有开发 API 和会话命名空间", () => {
  const config = resolveRuntimeConfig("develop");

  assert.equal(config.stage, "development");
  assert.equal(config.apiBaseUrl, "https://pay-dev.muybaby6.icu");
...
test("正式版拒绝非 production、非 HTTPS 和开发域名", () => {
  ...
  assert.throws(
    () => assertRuntimeConfig("release", releaseConfig({ apiBaseUrl: "https://pay-dev.muybaby6.icu" })),
    /正式版禁止使用开发 API 域名/
  );
  assert.throws(
    () => assertRuntimeConfig("release", releaseConfig({ apiBaseUrl: "https://api.pay-dev.example.com" })),
    /正式版禁止使用开发 API 域名/
  );
  assert.throws(

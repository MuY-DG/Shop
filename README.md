...
面向微信小程序渠道的单商家自营电商系统。当前定位是“单商家、单仓为主”，
采用 Spring Boot 模块化单体后端、Vue 3 管理后台和原生微信小程序。
...
| 交易 | 购物车、优惠券、下单、库存锁定、15 分钟支付截止时间、超时关单 | 单商家、单仓模型，不做跨商家或跨仓拆单 |
| 支付 | 微信支付、精确路由回调验签、退款恢复、按售后商品数量幂等回补库存、微信交易账单（`ALL`）下载与本地支付/退款日对账 | 不含微信资金账单或银行到账核对；真实商户账单链路仍需发布验收 |
| 履约 | 拆分包裹、部分发货、微信发货可靠投递、物流轨迹、电子面单 | 真实微信生产链路、插件和打印仍需发布验收 |
| 服务动态 | 微信新版 `2001` “购物（实体物流）服务动态”、可靠状态投递与主动对账；Admin 动态运行控制、CAS 与变更审计 | 不是传统订阅消息；必须先开 Capture 验收队列，再单独开 Worker，生产真实展示仍待真机验收 |
| 售后 | 商品/数量级申请与审批、部分/累计退款、仅退款、退货退款、商家退货地址、用户退货物流、商家验收/拒收、退货超时关闭 | 真实微信退款回调仍需逐环境发布验收 |
| 用户 | 微信登录、个人资料、地址、收藏、足迹、评价、客服与账号自助注销 | 会员等级、积分、储值为后续经营能力 |
...
第二代 Flyway 基线在 H2 和 MySQL 8.4 上直接建库，并由契约测试校验关键表、默认数据和
一次性 Super 引导哨兵。Admin 的自动导入声明和 ESLint globals 元数据纳入版本库；CI 会
在生产构建后校验生成结果无差异，保证全新 clone 不依赖预先启动 Vite。任何自动化结果
都不代替真实微信商户回调、交易账单、2001 服务动态或生产环境验收。

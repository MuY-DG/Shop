...
| 服务动态 | V94 微信新版 `2001` “购物（实体物流）服务动态”、可靠状态投递与主动对账 | 不是传统订阅消息；意图采集、外呼 Worker 和 SAFE+JSON 结果回调默认全部关闭，生产真实送达待验收 |
...
2026-08-10 本轮 V94 发布候选已记录的默认单元/H2 层为 1236 项，Docker/Testcontainers
集成层为 54 项，两层均为零失败、零错误、零跳过，分别用时 4 分 55 秒和 3 分 18 秒。
V94 聚焦测试 81 项通过；Failsafe 10 个必需套件共 54 项、零跳过，测试分层门禁确认
9 类 MySQL 8.4.10 与 1 类 Redis 7.4.9-alpine Testcontainers 套件全部执行。Flyway
V1-V94 共 94 个版本连续且无重复；Admin `pnpm check` 159 项和生产构建、

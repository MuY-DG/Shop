...
- 订单管理 → 物流配置中维护默认发货地址；电子面单停用时仍能保存。手动发货使用发货时的地址，电子面单使用制单时的地址，后续修改不影响旧包裹。
- 商品、SKU、库存、运费模板、优惠券和首页装修可以保存并重新读取。
- 敏感配置页面不回显私钥、APIv3 Key 或 COS Secret 明文。
- 微信平台保存时通过 access token 探测，COS 保存时通过临时对象读写探测，微信支付启用时通过签名查单探测。
- 上述探测通过不等于真实业务验收通过；支付回调、APIv3 解密、浏览器 CORS 和真实发货仍按下文验证。
...
- 待收货/部分发货卡片显示“查看物流”；订单详情只展示配送摘要，点击进入物流详情，切换包裹后运单和轨迹同步切换，售后入口仍可从订单详情进入。
- 有轨迹时配送摘要展示最新真实节点，收货地址保持不变；无轨迹时保留发货地址，失败时可重试并使用“查看全部物流”。旧手动包裹没有地址快照时显示缺省提示。
- 发货后订单卡片展示运输车、实际物流状态和最新节点；没有轨迹时显示“已发货 / 等待物流更新”，不虚构预计送达时间。多包裹显示包裹数量。
- 新申请的仅退款若全部数量尚未发货，会进入自动审核队列（默认每 5 秒扫描）；混有已发货数量、退货退款及上线前待审核单保留商家审核。自动审核和渠道请求应记录 SYSTEM 来源。
- 申请退款后详情页展示审核/退款进度并自动刷新；后台确认 REFUNDED 后跳转退款成功页。切后台或离开页面停止轮询，退款异常不显示成功；空凭证不展示，底部按钮文字居中。
- 整单退款完成后订单详情和列表均不显示物流入口；部分退款仍展示其余商品物流。模拟器页面验收与真实支付退款验收分别记录。

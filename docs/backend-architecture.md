...
3. 支付单持久化渠道配置 ID 和不可逆 SHA-256 指纹；查单、关单、退款、恢复及最终落账都再次校验该身份。新支付和新退款各自生成 192-bit 不可猜回调路由 token，微信收到的地址为固定 HTTPS 完整路径后追加 `/r/{token}`；最终 URL 会在支付单、退款单及业务状态落库前完成协议、端口和 255 字符上限预检。退款编排显式挂起调用方事务，先用无密钥快照排除非法状态，再在进入售后锁事务前完成配置解析及密钥读取；锁内用支付单 ID/配置 ID/指纹拒绝过期预检结果，准备、渠道调用和结果确认分别处于独立边界。回调在验签解密前通过 token 精确定位唯一支付/退款及历史配置，数据库查询命中后还会做常量时间精确 token 比较，解密后再严格比较商户支付单号/退款单号；未知、非法、未验签或尚未绑定业务身份的请求都不写业务回调日志。旧数据保持 token 为 `NULL` 并继续由固定端点兼容，routed 数据禁止从旧端点处理；关闭新 token 签发后产生的 legacy 退款仍按退款单自身的 token 模式枚举历史配置。数据库配置创建支付单前在同一事务内锁行复核渠道关键字段，配置并发变更会让准备阶段重试。已被支付单引用的数据库配置禁止原地修改。当前运行时只允许数据库支付配置；历史 ENV snapshot 仅为旧支付查询、退款、回调和对账提供只读回放，不能再创建或导入。
...
10. Admin 在浏览器内读取支付 PEM 文本，后端校验 RSA 材料后以带上下文 AAD 的密文直接写入数据库；日常配置不创建文件或对象存储资产。仅为已有数据库行保留一次性的旧 file-ID 转密文维护入口，并在事务外读取、短事务锁行复核后清空旧引用。

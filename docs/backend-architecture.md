...
3. 支付单持久化渠道配置 ID 和不可逆 SHA-256 指纹；查单、关单、退款、恢复及最终落账都再次校验该身份。支付回调还校验 `mchid/appid`、`transaction_id` 和 CNY 币种；从候选配置解密出商户单号后，会按支付/退款路由查询持久化身份，并用精确历史配置重新验签解密。数据库配置创建支付单前在同一事务内锁行复核渠道关键字段，配置并发变更会让准备阶段重试，封住“解析配置后、落支付单前”的竞态。已被支付单引用的数据库配置禁止原地修改。ENV 支付在创建支付单前，以该指纹为内容地址保存 AES-GCM 加密的 APIv3/商户私钥/微信公钥快照，轮换 `WECHAT_PAY_*` 后仍可恢复原配置；升级前无指纹/快照的 ENV 单继续失败关闭。根密钥 `SHOP_PAYMENT_SECRET_KEY` 必须由部署密钥系统稳定保管。
...
1. 为支付/退款回调增加不可猜的配置版本路由标识，不再依赖“当前配置 + 最近 32 个 DB/ENV 历史候选”；同时为 `SHOP_PAYMENT_SECRET_KEY` 设计显式 key-ring 与在线重加密流程，支持根加密密钥轮换。
2. 把总览、交易、用户、营销、客服等剩余报表的原始事实列表逐步下沉为 SQL 聚合，并以生产数据执行 `EXPLAIN ANALYZE` 校准组合索引和深分页策略。
3. 统一商品参数选项与 SPU 参数快照的并发锁顺序，评估从 JSON 快照迁移到规范化关联表，避免删除选项与商品保存并发时写入陈旧值。
4. 将文件引用维护为统一的资产使用账本，替代删除时锁住资产后跨多个业务表扫描（尤其富文本 `locate`）；同时继续拆分 `AdminProductService`、`CustomerServiceService` 等超大类。
5. 为图片上传增加按主体/IP 的速率与流量额度，并用进程级小型信号量限制同步 ImageIO 解码并发；当前字节、尺寸、帧数与累计像素限制约束单请求，但不能替代服务级容量保护。

...
2026-08-10 最近一次已记录的 V93 发布候选门禁快照：默认单元/H2 层 1179 项、集成层
53 项，均为 0 failures / 0 errors / 0 skipped；9 个必需集成套件报告齐全，测试分层
校验通过，Flyway V1-V93 共 93 个迁移连续且无重复。同期管理后台 `pnpm check`
159 项通过且生产构建成功，小程序 `pnpm check` 192 项通过。
...
- V92/V93 在生产 MySQL 实际存量数据上的升级、锁竞争与回滚/恢复演练；Testcontainers
...
- 支付、发货、退款、客服回复和低库存订阅消息尚未完成真实模板授权、用户同意和
  真机发送验收；发票、采购入库、盘点、多包裹和多仓仍属后续能力。
